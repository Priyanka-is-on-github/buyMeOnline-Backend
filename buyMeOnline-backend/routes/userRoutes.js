...
router.post("/signup", async (req, res) => {
    try {
        ...
        if (!validatePassword(password)) {
            return res.status(403).json({ status: "err", message: "A valid password must be at least 8 characters long, with at least one uppercase letter, one lowercase letter, one digit, and one special character (e.g., @, #, $)." });
        }
        ...
        const createdUser = await pool.query('INSERT INTO userinfo(name,email,password) VALUES($1,$2,$3) RETURNING *', [newUser.name, newUser.email, newUser.password]);
